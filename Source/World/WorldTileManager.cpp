...
bool WorldTileManager::AbleToLockThisTile(TileInfo* pLockTile, TileInfo* pTargetTile, bool ignoreEmpty)
{
    ...
    int32 neighbors[4][2] = {{1, 0}, {0, 1}, {-1, 0}, {0, -1}};
                                      ...
                                      std::vector<TileInfo*>& outTiles)
{
    ...
    while (totalLocked < tileSizeToLock && radius <= maxRadius)
    { // :/ is it really ok? is that O(n^3)???
...
bool WorldTileManager::RandomizeOuijaBoardTile(TileInfo* pTile)
{
    ...
    static std::vector<string> ouijaCommand = {"/furious", "/rolleyes", "/idk", "/omg",  "/wave", "dance",
                                               "/love",    "/sleep",    "/fp",  "/fold", "/yes",  "/no"};
