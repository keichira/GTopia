...
{
    static const std::unordered_map<string, uint32> itemConsumableFlag{{"NEED_TARGET", CONSUMABLE_FLAG_NEED_TARGET},
...
ItemInfoManager::ItemInfoManager() : m_version(0), m_itemCount(0), m_totalNegativeCount(0), m_totalPositiveCount(0)
{
    m_invalidItem.id = -65353;
    std::fill(std::begin(m_itemLookup), std::end(m_itemLookup), &m_invalidItem);
}
...
bool ItemInfoManager::Load(const string& filePath)
{
    ...
    m_totalPositiveCount = 0;
    m_totalNegativeCount = 0;
    sMaxPositiveID = 0;

    for (auto& line : cfg.Lines())
    {
        const string& key = line.GetString(0);
        if (key == "add_item" || key == "add_cloth")
        {
            int32 rawId = line.GetInt(1);
            if (rawId >= 0)
            {
                m_totalPositiveCount += 2;
                if (rawId > sMaxPositiveID)
                {
                    sMaxPositiveID = rawId;
                }
            }
            else
            {
                m_totalNegativeCount += 2;
            }
        }
    }

    if (sMaxPositiveID % 2 == 0)
    {
        sMaxPositiveID++;
    }

    m_items.reserve(m_totalPositiveCount + m_totalNegativeCount);
    int32 lastItemID = 0;
    usize activeItemIdx = -1;
    usize activeSeedIdx = -1;
    ...
    for (auto& line : cfg.Lines())
    {
        ...
        if (key == "add_item")
        {
            ...
            lastItemID = item.id;
            m_items.push_back(std::move(item));
            activeItemIdx = m_items.size() - 1;

            CreateDefaultSeedForItem(&m_items.back());
            activeSeedIdx = m_items.size() - 1;
            continue;
        }
        ...
        if (key == "set_seed")
        {
            ...
            if (activeSeedIdx != -1)
            {
                ItemInfo* pSeed = &m_items[activeSeedIdx];
                pSeed->seed1 = line.GetUInt(1);
                pSeed->seed2 = line.GetUInt(2);
                pSeed->seedBgColor = ToColor(line.GetString(3), ',');
                pSeed->seedFgColor = ToColor(line.GetString(4), ',');
            }
            continue;
        }
        ...
        ItemInfo* pActiveItem = &m_items[activeItemIdx];

        if (key == "description")
        {
            pActiveItem->description = line.GetString(1);
        }
        if (key == "set_element")
        {
            pActiveItem->element = StrToItemElement(line.GetString(1));
            continue;
        }
        if (key == "set_flags")
        {
            for (uint16 i = 1; i < line.GetArgSize(); ++i)
            {
                if (line.IsEmpty(i))
                    continue;
                pActiveItem->flags |= StrToItemFlag(line.GetString(i));
            }
            continue;
        }
        if (key == "set_flags2")
        {
            for (uint16 i = 1; i < line.GetArgSize(); ++i)
            {
                if (line.IsEmpty(i))
                    continue;
                pActiveItem->flags2 |= StrToFlags2(line.GetString(i));
            }
            continue;
        }
        if (key == "set_fx_flags")
        {
            ...
            while (i < line.GetArgSize())
            {
                ...
                if (convFlag == ITEM_FX_FLAG_MULTI_ANIM2)
                {
                    ...
                    {
                        pActiveItem->multiAnim2 += line.GetString(i) + "|";
                        ++i;
                    }
                    ++i;
                    continue;
                ...
                if (convFlag == ITEM_FX_FLAG_DUAL_LAYER)
                {
                    ...
                    pActiveItem->dualAnimLayer.x = ToInt(dualLayer[0]);
                    pActiveItem->dualAnimLayer.y = ToInt(dualLayer[1]);
                    continue;
                }
                if (convFlag == ITEM_FX_FLAG_OVERLAY_OBJECT)
                {
                    if (i >= line.GetArgSize())
                        break;
                    pActiveItem->overlayTextureFile = line.GetString(i++);
                    continue;
                }
                if (convFlag == ITEM_FX_FLAG_RENDER_FX_VARIANT_VERSION)
                {
                    if (i >= line.GetArgSize())
                        break;
                    pActiveItem->variantVersionItem = line.GetInt(i++);
                    continue;
                }
            }
            continue;
        ...
        if (key == "set_extra")
        {
            pActiveItem->extraString = line.GetString(1);
            pActiveItem->animMS = line.GetInt(2, 200);
            pActiveItem->extraStringHash = line.GetUInt(3);
            continue;
        }
        if (key == "set_max_hold")
        {
            pActiveItem->maxCanHold = line.GetUInt(1, 1);
            continue;
        }
        if (key == "set_custom_punch")
        {
            pActiveItem->customizedPunchParameters = line.GetString(1);
            continue;
        }
        if (key == "set_config_name")
        {
            pActiveItem->configName = line.GetString(1);
            continue;
        }
        if (key == "set_rarity")
        {
            pActiveItem->rarity = line.GetInt(1, 1);
            if (activeSeedIdx != -1)
                m_items[activeSeedIdx].rarity = pActiveItem->rarity;
            continue;
        }
    }
    ...
    {
        uint16 slot = (uint16)(BASE_INDEX + item.id);
        m_itemLookup[slot] = &item;

        if (item.id < 0)
        {
            uint16 clientSlot = (uint16)(BASE_INDEX + sMaxPositiveID + Abs(item.id));
            m_itemLookup[clientSlot] = &item;
        }
    }
...
bool ItemInfoManager::LoadByItemsDat(const string& filePath, bool database)
{
    File file;
    ...
    {
        item.Serialize(memBuffer, true, database, m_version);
    }
...
void ItemInfoManager::SaveToClientData(bool forOgg, uint16 minVersion, uint16 maxVersion)
{
    ...
    for (uint16 i = minVersion; i < maxVersion; ++i)
    {
        ...
        for (auto& item : m_items)
        {
            item.Serialize(memSizeBuffer, true, false, i);
        }
...
ItemInfo* ItemInfoManager::GetSpliceInfo(int16 seed1, int16 seed2)
{
    int16 a = Min(seed1, seed2);
    int16 b = Max(seed1, seed2);
    int32 key = (int32(a) << 16) | int32(b);
...
ConsumableInfo* ItemInfoManager::GetConsumableInfo(int32 itemID)
{
...
std::vector<int16> ItemInfoManager::GetSortedItemIdsByName(const string& name, bool skipSeeds)
{
    std::vector<int16> sortedItems;
    if (name.empty())
        return sortedItems;
    ...
    for (auto& item : m_items)
    {
        if (skipSeeds && item.type == ITEM_TYPE_SEED)
            continue;
...
uint32 ItemInfoManager::GetItemRarity(int32 itemID)
{
    ItemInfo* pItem = GetItemByID(itemID);
    if (!pItem)
        return 0;
...
void ItemInfoManager::SetupItemExtras()
{
    for (auto& item : m_items)
    {
        ...
        ItemInfo* pSeed = &item;
        ItemInfo* pItem = GetItemByID(GetBaseItemID(pSeed->id));
        if (!pItem)
            continue;
...
void ItemInfoManager::CreateDefaultSeedForItem(ItemInfo* pItem)
{
    ...
    if (pItem->HasFlag(ITEM_FLAG_RANDGROW))
        seed.flags |= ITEM_FLAG_RANDGROW;
    if (pItem->HasFlag(ITEM_FLAG_MOD))
        seed.flags |= ITEM_FLAG_MOD;
    if (pItem->HasFlag(ITEM_FLAG_SEEDLESS))
        seed.flags |= ITEM_FLAG_SEEDLESS;
    if (pItem->HasFlag(ITEM_FLAG_BETA))
        seed.flags |= ITEM_FLAG_BETA;
    if (pItem->HasFlag(ITEM_FLAG_HOLIDAY))
        seed.flags |= ITEM_FLAG_HOLIDAY;

    int32 clientID = ToItemClientID(pItem->id);
    seed.seedBg = (uint8)((clientID / 2) % 16);
    seed.seedFg = (uint8)(clientID % 16);
    seed.treeBg = (uint8)((clientID / 2) % 8);
    seed.treeFg = (uint8)(clientID % 8);

    seed.id = (pItem->id >= 0) ? (pItem->id + 1) : (pItem->id - 1);
    ...
    switch (pItem->id)
    {
        ...
        case ITEM_ID_COMET_DUST:
            seed.name = "Starseed";
            break;
        case ITEM_ID_ANTIMATTER_DUST:
            seed.name = "Galactic Starseed";
            break;
        case ITEM_ID_MUTATED_SEED_CORE:
            seed.name = "Mutated Seed";
            break;
        default:
            seed.name = pItem->name + " Seed";
    }
    ...
    m_items.push_back(std::move(seed));
}
...
int32 GetBaseItemID(int32 itemID)
{
    ...
    uint8 isSeed = (itemID >= 0) ? (itemID % 2 == 1) : (itemID % 2 == 0);

    if (itemID >= ITEM_ID_CUDDLY_BUNNY && itemID <= ITEM_ID_PSYCHOTIC_BUNNY_SEED)
        return ITEM_ID_CUDDLY_BUNNY + isSeed;
    if (itemID >= ITEM_ID_RED_GROWSABER && itemID <= ITEM_ID_DOUBLE_GROWSABER_SEED)
        return ITEM_ID_RED_GROWSABER + isSeed;
    if (itemID >= ITEM_ID_FASHIONABLE_DRESS_BLUE && itemID <= ITEM_ID_FASHIONABLE_DRESS_YELLOW_SEED)
        return ITEM_ID_FASHIONABLE_DRESS_BLUE + isSeed;
    if (itemID >= ITEM_ID_STYLIN_DRESS_BLUE && itemID <= ITEM_ID_STYLIN_DRESS_YELLOW_SEED)
        return ITEM_ID_STYLIN_DRESS_BLUE + isSeed;
    if (itemID >= ITEM_ID_NUMBER_0 && itemID <= ITEM_ID_NUMBER_9_SEED)
        return ITEM_ID_NUMBER_0 + isSeed;
    if (itemID >= ITEM_ID_PAINT_BUCKET_RED && itemID <= ITEM_ID_PAINTBRUSH_SEED)
        return ITEM_ID_PAINT_BUCKET_RED + isSeed;
    if (itemID >= ITEM_ID_RED_CRAYON && itemID <= ITEM_ID_PURPLE_CRAYON_SEED)
        return ITEM_ID_RED_CRAYON + isSeed;
    if (itemID >= ITEM_ID_SUPER_SHIRT_RED && itemID <= ITEM_ID_SUPER_EYE_MASK_BLACK_SEED)
        return ITEM_ID_SUPER_SHIRT_RED + isSeed;
    if (itemID >= ITEM_ID_SUPER_LOGO_SKULL && itemID <= ITEM_ID_SUPER_LOGO_ECHO_SEED)
        return ITEM_ID_SUPER_LOGO_SKULL + isSeed;
    if (itemID >= ITEM_ID_SUPERPOWER_HEAT_VISION && itemID <= ITEM_ID_SUPERPOWER_OVERHEAT_SEED)
        return ITEM_ID_SUPERPOWER_HEAT_VISION + isSeed;
    if (itemID >= ITEM_ID_SUPERPOWER_ICE_SHARDS && itemID <= ITEM_ID_SUPERPOWER_FROZEN_MIRROR_SEED)
        return ITEM_ID_SUPERPOWER_ICE_SHARDS + isSeed;
    if (itemID >= ITEM_ID_TANGRAM_BLOCK_A && itemID <= ITEM_ID_TANGRAM_BLOCK_L_SEED)
        return ITEM_ID_TANGRAM_BLOCK_A + isSeed;
    if (itemID >= ITEM_ID_CARD_BLOCK_SPADE && itemID <= ITEM_ID_CARD_BLOCK_DIAMOND_SEED)
        return ITEM_ID_CARD_BLOCK_SPADE + isSeed;
    if (itemID >= ITEM_ID_SUPERPOWER_SUPER_STRENGTH && itemID <= ITEM_ID_SUPERPOWER_REGENERATION_SEED)
        return ITEM_ID_SUPERPOWER_SUPER_STRENGTH + isSeed;
    if (itemID >= ITEM_ID_SUPERPOWER_SHOCKING_FIST && itemID <= ITEM_ID_SUPERPOWER_RESUSCITATE_SEED)
        return ITEM_ID_SUPERPOWER_SHOCKING_FIST + isSeed;
    if (itemID >= ITEM_ID_STATUE_BLOCK && itemID <= ITEM_ID_STATUE_NOSE_SEED)
        return ITEM_ID_STATUE_BLOCK + isSeed;
    if (itemID >= ITEM_ID_WIZARD_HAT && itemID <= ITEM_ID_CURSED_WIZARD_HAT_SEED)
        return ITEM_ID_WIZARD_HAT + isSeed;
    if (itemID >= ITEM_ID_HIGH_HEELS_BLUE && itemID <= ITEM_ID_HIGH_HEELS_YELLOW_SEED)
        return ITEM_ID_HIGH_HEELS_BLUE + isSeed;
    if (itemID >= ITEM_ID_FASHION_PURSE_BLUE && itemID <= ITEM_ID_FASHION_PURSE_YELLOW_SEED)
        return ITEM_ID_FASHION_PURSE_BLUE + isSeed;
    if (itemID >= ITEM_ID_NUMBER_BLOCK_0 && itemID <= ITEM_ID_NUMBER_BLOCK_9_SEED)
        return ITEM_ID_NUMBER_BLOCK_0 + isSeed;
    if (itemID >= ITEM_ID_SURGICAL_SPONGE && ITEM_ID_SURGICAL_STITCHES_SEED)
        return ITEM_ID_SURGICAL_SPONGE + isSeed;
    if (itemID >= ITEM_ID_SURGICAL_PINS && ITEM_ID_SURGICAL_LAB_KIT_SEED)
        return ITEM_ID_SURGICAL_SPONGE + isSeed;
