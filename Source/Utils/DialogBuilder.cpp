...
DialogBuilder& DialogBuilder::AddButton(const string& label, const string& buttonID, float scale)
{
    m_str += "add_button|" + label + "|" + buttonID + "|" + ToString(scale) + "\n";
    return *this;
}

DialogBuilder& DialogBuilder::AddFloater(const string& worldName, uint32 playerCount, float scale, uint32 color)
{
    m_str +=
        "add_floater|" + worldName + "|" + ToString(playerCount) + "|" + ToString(scale) + "|" + ToString(color) + "\n";
    return *this;
}

DialogBuilder& DialogBuilder::AddFloater(const string& worldName, uint32 playerCount, float scale, Color& color)
{
    AddFloater(worldName, playerCount, scale, color.GetAsUINT());
    return *this;
}

DialogBuilder& DialogBuilder::AddHeading(const string& title)
{
    m_str += "add_heading|" + title + "\n";
    return *this;
}

DialogBuilder& DialogBuilder::SetMaxRows(uint32 maxRowCount)
{
    m_str += "set_max_rows|" + ToString(maxRowCount) + "\n";
    return *this;
}
